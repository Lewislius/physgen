...
RUNTIME_TMPDIR="${V1_ROOT}/tmp/ace_router_m1"
...
SEEDS="${SEEDS:-42}"
RUN_ID="${RUN_ID:-m1-$(date -u +%Y%m%d-%H%M%S)}"
...
printf '[RUN] method=M1 pure Wan2.2-TI2V-5B I2V + 1280x704 first frame + original short prompt\n'
printf '[RUN] cplus/cminus are not encoded or injected\n'

...
RUNTIME_TMPDIR="${V1_ROOT}/tmp/ace_router_m0"
...
SEEDS="${SEEDS:-42}"
RUN_ID="${RUN_ID:-m0-$(date -u +%Y%m%d-%H%M%S)}"
...
T2V_ORIENTATION="${T2V_ORIENTATION:-landscape}"
...
cmd=(
  ...
  --t2v_orientation "${T2V_ORIENTATION}"
)
...
printf '[RUN] method=M0 pure Wan2.2-TI2V-5B T2V + original short prompt\n'
printf '[RUN] no image/cplus/cminus enters the model; orientation_policy=%s\n' "${T2V_ORIENTATION}"
printf '[RUN] samples=%s seeds=%s frame_num=%s max_area=%s steps=%s cfg=%s\n' "${SAMPLE_IDS}" "${SEEDS}" "${FRAME_NUM}" "${MAX_AREA}" "${SAMPLING_STEPS}" "${GUIDE_SCALE}"

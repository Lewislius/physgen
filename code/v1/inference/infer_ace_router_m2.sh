...
RUNTIME_TMPDIR="${V1_ROOT}/tmp/ace_router_m2"
...
SEEDS="${SEEDS:-42}"
RUN_ID="${RUN_ID:-m2-$(date -u +%Y%m%d-%H%M%S)}"
...
printf '[RUN] method=M2 Wan2.2-TI2V-5B I2V + 1280x704 first frame + cplus-only residual\n'
printf '[RUN] residual=positive-semantic; cminus is not encoded or injected\n'
printf '[RUN] samples=%s seeds=%s lambda0=%s preset=%s\n' "${SAMPLE_IDS}" "${SEEDS}" "${LAMBDA0}" "${LAYER_PRESET}"

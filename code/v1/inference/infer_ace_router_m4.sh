...
RUNTIME_TMPDIR="${V1_ROOT}/tmp/ace_router_m4"
...
SEEDS="${SEEDS:-42}"
RUN_ID="${RUN_ID:-m4-$(date -u +%Y%m%d-%H%M%S)}"
...
FPS="${FPS:-24}"
LAMBDA0="${LAMBDA0:-0.08}"
RESIDUAL_CAP_RATIO="${RESIDUAL_CAP_RATIO:-0.015}"
LAYER_PRESET="${LAYER_PRESET:-mid_b}"
STEP_PRESET="${STEP_PRESET:-safe_t2v_v1}"
...
M4_ORIENTATION="${M4_ORIENTATION:-landscape}"
...
printf '[RUN] method=M4 T2V + compiled metadata semantic + safe causal residual\n'
printf '[RUN] no image tensor enters T5, DiT, VAE, or scheduler; orientation_policy=%s\n' "${M4_ORIENTATION}"

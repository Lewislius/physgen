...
RUNTIME_TMPDIR="${V1_ROOT}/tmp/ace_router_m4_initial_v1_hd97"
...
SEEDS="${SEEDS:-42}"
RUN_ID="${RUN_ID:-m4-initial-v1-hd97-$(date -u +%Y%m%d-%H%M%S)}"
...
printf '[RUN] M4 initial ACE-Router V1 compatibility ablation at 1280x704 / 97 frames\n'
...
printf '[RUN] fixed landscape T2V; samples=%s seeds=%s\n' "${SAMPLE_IDS}" "${SEEDS}"

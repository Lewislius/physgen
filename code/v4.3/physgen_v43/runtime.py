...
def read_config(path):
    ...
    config["paths"] = {key: os.path.expandvars(value.replace("${V43_ROOT}", str(ROOT)))
    ...
    for parameters in groups.values():
        for parameter in parameters:
            if parameter.grad is None:
                raise RuntimeError('An active V4.3 parameter has no gradient; check loss/state connectivity')
...
def save_checkpoint(config, corrector, optimizer, scheduler, step, cursor, metrics, final=False):
    ...
    if rank == 0:
        ...
        if config['corrector'].get('architecture') == 'v43_native_joint_flow_oracle_v1':
            variant = 'flow_oracle' if config['loss']['oracle_weight'] > 0 else 'fm_struct'
            pointer = Path(config['paths']['checkpoint_root']) / f'v43_{variant}_latest.json'
            temporary = pointer.with_suffix('.tmp')
            temporary.write_text(json.dumps(dict(checkpoint=str(destination), next_step=step,
                architecture='v43_native_joint_flow_oracle_v1', loss_variant=variant, run_name=config['run_name']), indent=2))

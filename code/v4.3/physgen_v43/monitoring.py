...
def normalize_logging(config):
    ...
    wandb.setdefault('project', 'physgen-v43')
...
def checkpoint_source_files(root):
    root = Path(root)
    for directory in ('physgen_v43', 'train', 'inference', 'tools', 'configs', 'evaluation'):
...
class Logger:
    ...
    def step(self, step, metrics, validation=False):
        ...
        summary = ('validation_loss',) if validation else (
            'loss/total', 'loss/fm', 'loss/struct', 'loss/oracle', 'loss/weighted_fm',
            'loss/weighted_struct', 'loss/weighted_oracle', 'learning_rate',
            'grad/global_norm_before_clip', 'sigma', 'oracle/acceptance',

"""V4.3 sampling: native-width P/H, reset P, corrected positive and negative CFG branches."""
import argparse
...
from physgen_v43.runtime import ROOT, read_config, seed_all
from physgen_v43.corrector import Corrector, ARCHITECTURE
from physgen_v43.config import validate_config
from physgen_v43.backbone import load_wan, ProcessWan
from physgen_v43.coordinates import Coordinates
from physgen_v43.data import letterbox
from physgen_v43.encoders import load_vae, load_text_encoder, DifferentiableDecoder
from physgen_v43.views import choose_canvas
...
def resolve_checkpoint(value):
    if str(value) in ('latest', 'latest_flow_oracle', 'latest_fm_struct'):
        variant = 'fm_struct' if str(value) == 'latest_fm_struct' else 'flow_oracle'
        pointer = ROOT / f'checkpoints/v43_{variant}_latest.json'
        if not pointer.is_file():
            raise FileNotFoundError('No trained V4.3 checkpoint exists yet. Train this architecture first.')
        saved = json.loads(pointer.read_text())
        if saved['architecture'] != ARCHITECTURE or saved.get('loss_variant') != variant:
...
def state_policy(config, args):
    ...
    if requested is False:
        raise ValueError("V4.3 was trained with reset P; persistent inference is unsupported")
...
def load_checkpoint(checkpoint):
    ...
    if not (checkpoint / 'corrector.pt').is_file():
        raise FileNotFoundError(f"V4.3 checkpoint required: {checkpoint}/corrector.pt")
    config = validate_config(read_config(checkpoint / "config.yaml"))
    if config["stage"] != "A":
    ...
    report = dict(
        ...
        p_working_width=config["corrector"]["hidden_width"], jepa_width=config["corrector"]["jepa_width"],
        joint_depth=config["corrector"]["depth"], enable_b=False, strict_load=True,
        ...
        training_gt_conditioned=False, inference_oracle=False,
    )
...
def validate_writers(model, writer_off):
    valid = {"A"} | {label for _, _, label in model.correction_sites()}
...
def check_checkpoint(args):
    ...
    sites = {f"A@{n}": unit for n, unit in corrector.A.items()}
...
def build_parser():
    ...
    parser.add_argument("--variant", choices=("full", "wan", "A"), default="full",
                        help="full/A enables the three correctors; wan disables all correctors")
    parser.add_argument("--writer-off", nargs="*", default=[], help="A or individual sites: A@5 A@15 A@25")

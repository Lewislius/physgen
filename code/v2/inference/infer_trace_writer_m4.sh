...
mkdir -p "${V2_ROOT}/tmp/trace_m4" "${V2_ROOT}/.cache/huggingface" "${V2_ROOT}/.cache/torch"
...
RUN_ID="${RUN_ID:-trace-m4-$(date -u +%Y%m%d-%H%M%S)}"
...
export XDG_CACHE_HOME="${V2_ROOT}/.cache" TMPDIR="${V2_ROOT}/tmp/trace_m4"
...
printf '[RUN] TRACE-M4 T2V fixed5: plan only, no first-frame model input\n'
printf '[RUN] samples=%s seeds=%s crossfade=%s spatial=time\n' "${SAMPLE_IDS:-all}" "${SEEDS:-42}" "${CROSSFADE_TOKENS:-1}"

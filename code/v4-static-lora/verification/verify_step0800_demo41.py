...
def main():
    destination = ROOT / "verification/standalone_20260918"
    ...
    previous = read_json(ROOT / "tests/fixtures/demo41.json")["cases"]
    assert [(c["id"], c["mode"], c["prompt"], c.get("image")) for c in plan["cases"]] == [
        (c["id"], c["mode"], c["prompt"], c.get("image")) for c in previous]

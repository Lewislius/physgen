...
class InferenceTests(unittest.TestCase):
    ...
    def test_demo41_matches_v4_prompt_image_mode_and_order(self):
        ...
        previous = fixtures.import_file("lora_test_v4_cases", ROOT.parent / "v4/inference/cases.py")
        reference = previous.collect_cases(SimpleNamespace(
            suite="both", image=self.cfg["paths"]["reference_image"], prompt=self.cfg["paths"]["reference_prompt"],
            mode="i2v", demo_root=self.cfg["paths"]["demo_root"], sample_ids="all", demo_modes="both"))
        self.assertEqual([(c["id"], c["mode"], c["prompt"], c.get("image")) for c in cases],
                         [(c["case_id"], c["mode"], c["prompt"], c.get("image")) for c in reference])

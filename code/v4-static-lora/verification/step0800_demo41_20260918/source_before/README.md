...
两个任务均使用 `blk-96g` 单卡、原 `moviestory` 环境、相同镜像与 HOME/NAS 挂载。默认最终推理读取本目录的 `checkpoints/latest_final.json`，使用 step1200 EMA。
...
| 推理 | 相同 81 个 final 案例、UniPC 50 步、shift=5、CFG=5、seed=42；正/负条件分支均使用带 LoRA 的 Wan |
...
- `inference_outputs/final_step1200_ema_seed42/`：每案例 MP4、latent、抽帧图片、采样轨迹和元数据，以及总 `report.json`。本版不运行 JEPA teacher 评估或输出不存在的自定义状态指标。
...
恢复会校验 LoRA 结构、配置、源码、基模资产与数据身份，并恢复 optimizer、学习率日程、EMA、随机状态和采样队列。只能恢复本实验 checkpoint，不能把 v4.2 的 adapter checkpoint 当作 LoRA。保持本目录和实际复用的 `v4.2/physgen_v42` 源码稳定，以保证恢复与推理的源码校验一致。

可以指定任意已经完整保存的百步 checkpoint 做推理；设置独立 OUTPUT 防止混入其他 checkpoint 的结果：

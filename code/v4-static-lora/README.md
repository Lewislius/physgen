Wan2.2-TI2V-5B 的独立纯 LoRA、仅 FM 对比实验。通用 Python 实现、启动环境脚本、checkpoint、日志和推理结果均由本目录管理，不导入任何兄弟实验的源码。训练及推理直接走原生 Wan 前向，只给注意力线性层增加标准 `W(x) + (alpha/r)·B(A(x))`。

新增两种**帧内全注意力、帧间因果注意力**对比方法：`frame_causal_full`（全部历史帧）和 `frame_causal_window_k5`（前 5 个压缩帧加当前整帧）。两种方法均配有 1×96GB / 4×48GB 的训练 sh、Determined YAML，checkpoint 名自动包含方法、规格和 UTC 时间戳。配置、提交命令及注意力范围见 [frame_causal_lora/README.md](frame_causal_lora/README.md)。下文继续介绍原全注意力基线。

默认 step0800 / demo41 推理无需 `v4.2` 目录：Wan/T5/VAE/几何/运行时实现在 `static_lora/`，固定资产清单及负提示词编码在 `assets/`。外部资源只有已安装的 moviestory 环境、原 Wan 代码/权重和原 41 条测试图片/文本。历史训练配置仍记录原 VAE/T5 数据缓存的位置；仅重新训练或显式 `SUITE=final` 的训练/验证诊断需要这些数据文件。已有 checkpoint 文件不作修改。
...
独立运行修复和验证见 [inference/README.md](inference/README.md)；[最新测试日志](verification/standalone_20260918/unit_tests.log)包含保存/续训、真实 EMA 预检和禁止所有 v4.2 访问的独立性检查。
...
最新验证结果见 [verification/standalone_20260918/result.json](verification/standalone_20260918/result.json)。CPU 数值测试使用真实 Wan 类的缩小模型和 CPU SDPA；完整尺寸 LoRA 加载使用 meta 基模结构。推理流程测试使用实际 UniPC 和视频导出，预测器/VAE 为小型替身。完整 5B 的 GPU 训练、显存峰值和生成质量需要在 Determined 实际任务中验证。

...
from static_lora import ROOT, TRAINING_RECIPE
...
class LoRATests(unittest.TestCase):
    ...
    def test_source_mode_noise_queues_match_formal_v42_for_all_1200_steps(self):
        ...
        plan = ExposurePlan(records, 20260916)
        batches = []
        for step in range(1, 1201):
            actual = plan.batch(step)
            self.assertEqual(len(actual), 8)
            self.assertEqual(sum(e["mode"] == "i2v" for e in actual), 6)
            self.assertEqual(sum(e["low_noise"] for e in actual), 1)
            batches.append([dict(index=a["index"], mode=a["mode"], exposure=a["exposure"],
                                 temporal=a["low_noise"], repair=a["low_noise"] and step >= 801) for a in actual])
            for a in actual:
        ...
        golden = read_json(ROOT / "tests/fixtures/exposure_plan.json")
        self.assertEqual(digest(batches), golden["batches_sha256"])
    ...
    def test_final_inference_cases_exactly_match_formal_v42(self):
        ...
        self.assertEqual([(c["group"], c["id"], c["mode"]) for c in actual[:40]],
                         [(split, c["id"], c["mode"]) for split in ("train", "validation")
                          for c in manifest["diagnostics"][split]])
        expected = read_json(ROOT / "tests/fixtures/demo41.json")["cases"]
        self.assertEqual([(c["id"], c["prompt"], c.get("image"), c["mode"]) for c in actual[40:]],
                         [(c["id"], c["prompt"], c.get("image"), c["mode"]) for c in expected])

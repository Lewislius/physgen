...
from physgen_v4.write_supervision import paired_input, native_write_loss, joint_gradient_diagnostics
...
def configured_run(args):
    ...
    if args.resume:
        ...
        if requested['corrector'].get('architecture') == 'v4_fullwidth3_write_v1':
            if (config['corrector'].get('architecture') != 'v4_fullwidth3_write_v1'
                    or config['loss']['prior_weight'] != requested['loss']['prior_weight']):
                raise ValueError('RESUME must retain the same native architecture and PRIOR variant. '
                                 'Use INIT_FROM for an explicitly new run with compatible model weights.')
...
def runtime_overrides(config, args):
    ...
    cfg = config['train']
    if config['corrector'].get('architecture') == 'v4_fullwidth3_write_v1':
        if config['stage'] != 'A' or config['loss']['out_weight'] != 0:
            raise ValueError('The restored recipe has exactly FM/STRUCT/PRIOR/WRITE, with three A correctors')
        if config['loss'].get('write_weight', 0) <= 0:
            raise ValueError('The restored four-loss recipe must enable WRITE')
        if cfg.get('state_warmup_every', 0) != 0:
            raise ValueError('Retain the validated V4 reset-state training path')
        if config['data'].get('training_mode') != 'i2v':
            raise ValueError('The restored recipe trains only I2V, including image-conditioned CFG dropout')
...
def _train(args, cleanup):
    ...
    with trace.stage('dataset_init'):
        dataset = CachedWISA(config["paths"]["cache_root"], "train", limit=cfg.get("sample_limit", 0), config=config)
    ...
    for step in range(start, end):
        with trace.iteration(step + 1, step - start):
            ...
            b_active = False
            diagnostic_every = config['diagnostics'].get('loss_grad_every', 0)
            diagnose_pending = bool(diagnostic_every and (step == 0 or (step + 1) % diagnostic_every == 0))
            for micro in range(cfg["accumulation_steps"]):
                ...
                with torch.autocast("cuda", dtype=torch.bfloat16, cache_enabled=False):
                    ...
                    with trace.stage('forward'):
                        prediction, next_process, info = model(noisy, sample["first"], text, sigma, coords, process,
                            capture_write=config['loss'].get('write_weight', 0) > 0,
                            **({} if warm_active else gt_inputs))
                    if warm_active and prior_supervision_enabled(config):
                        info['prior'] = prior
                    info['prior_gt_conditioned'] = gt_active
                    write_value, write_metrics, repair_active = None, {}, False
                    if config['loss'].get('write_weight', 0) > 0:
                        with trace.stage('write_paired_input'):
                            repair_noisy, repair_active = paired_input(sample['latent'], noisy, sample['first'],
                                                                      sigma, config['write_supervision'])
                        if repair_active:
                            # Identical P0/text/sigma; only the latent is perturbed.
                            # Do not let this no_grad pass populate the AMP weight cache.
                            with torch.no_grad(), trace.stage('write_reference_forward'):
                                _, _, repair_info = model(repair_noisy, sample['first'], text, sigma, coords,
                                    info['initial'], capture_write=True)
                            repair_pairs = repair_info['write_pairs']
                        else:
                            repair_pairs = info['write_pairs']
                        with trace.stage('write_loss'):
                            write_value, write_metrics = native_write_loss(corrector, info['write_pairs'],
                                repair_pairs, sigma, coords, sample['first'] is not None)
                        del repair_pairs, repair_noisy
                        if repair_active:
                            del repair_info
                    with trace.stage('loss'):
                        loss, metrics, terms = training_loss(prediction, noise, sample["latent"], sample["first"], noisy,
                            sigma, info, sample["target"], sample["target_weight"], step, config, out_active, decoder, teacher,
                            write_loss=write_value, return_terms=True)
                        metrics.update(write_metrics)
                    # Prefer a nontrivial WRITE pair; a clean self-pair has zero
                    # WRITE gradient and cannot reveal FM/WRITE alignment.
                    if diagnose_pending and (repair_active or micro == cfg['accumulation_steps'] - 1):
                        with trace.stage('loss_gradient_diagnostics'):
                            metrics.update(joint_gradient_diagnostics(terms, groups))
                            metrics['loss_grad/micro_index'] = float(micro)
                            metrics['loss_grad/perturbed_pair'] = float(repair_active)
                ...
                metrics.update({"time/data_seconds": data_seconds, "time/forward_seconds": forward_seconds,
                    ...
                    "compute/wan_forwards": 1 + int(warm_active) + int(repair_active),
                    "write/paired_input_active": float(repair_active),
                    "data/i2v": 1.0,
                ...
                cursor += world
                del loss, terms, write_value, prediction, next_process, info, process, prior, gt_inputs, noisy, sample
...
def main():
    ...
    parser.add_argument("--config", default=str(ROOT / "configs/stability_v4_fullwidth3.yaml"))
    parser.add_argument("--phase", choices=("A1", "A2", "A3", "B1", "AB"), default="A1")
    parser.add_argument("--hardware", choices=("1x96g", "1x24g", "4x48g", "4x80g", "4x96g"), default="1x96g",
    ...
    parser.add_argument('--check-config', action='store_true', help='Read config/cache and count native parameters on CPU')
    ...
    if args.check_config:
        ...
        corrector.set_stage(config['stage'])
        dataset = CachedWISA(config['paths']['cache_root'], 'train', limit=config['train']['sample_limit'], config=config)
        sample = dataset[0]
        assert sample['target'].shape[-1] == 1664
        report = dict(architecture=config['corrector'].get('architecture'),
            sites=list(corrector.a_blocks), corrector_width=1664, corrector_heads=26,
            trainable_parameters={name: sum(p.numel() for p in ps) for name, ps in corrector.parameter_groups().items()},
            total_parameters=sum(p.numel() for p in corrector.parameters()),
            losses=config['loss'], train_samples=len(dataset), cache=config['paths']['cache_root'],
            global_batch=config['train']['effective_global_batch_size'],
            frames=config['data']['max_frames'], teacher_frames=config['data']['teacher_frames'],
            teacher_sampling=config['data'].get('teacher_sampling', 'uniform'),
            training_mode=config['data'].get('training_mode', 'i2v'),
            prior_width=config['corrector']['prior_width'], note='P0 initializer matches original V4; all correctors use 1664')

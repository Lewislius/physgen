...
def teacher_indices(frames, teacher_frames, sampling="uniform"):
    if sampling == "adjacent_pairs":
        if frames < 2 or teacher_frames < 2 or teacher_frames % 2:
            raise ValueError("Adjacent-pair teacher sampling needs >=2 frames and an even teacher frame count")
        starts = torch.linspace(0, frames - 2, teacher_frames // 2).round().long()
        return torch.stack((starts, starts + 1), dim=1).flatten()
    if sampling != "uniform":
        raise ValueError(f"Unknown teacher sampling: {sampling}")
...
class Coordinates:
    ...
    @classmethod
    def build(cls, times, height, width, teacher_frames, seconds_scale, teacher_size=384, teacher_sampling="uniform"):
        device = times.device
        latent_t = times[::4]
        indices = teacher_indices(times.numel(), teacher_frames, teacher_sampling).to(device)

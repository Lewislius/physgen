...
def loss_breakdown(metrics, weights, aux_alpha):
    """Report weighted scalar terms before gradient clipping/projection.
    ...
    write = sum(m.get('weighted_write',0.) for m in metrics)
    ...
    terms = dict(
        ...
        struct=dict(active=True, base_weight=weights["struct"], aux_alpha=aux_alpha,
                    ...
                    noise_weighted_mean=struct_nominal / weights["struct"] if weights["struct"] else 0.,
    ...
    if metrics[0].get("struct_scheme") == "v4_limits":
        terms["struct"].update(active=weights["struct"] > 0, scheme="v4_limits", warmup=metrics[0]["struct_warmup"],
                               effective_weight=weights["struct"] * metrics[0]["struct_warmup"],
                               micro_weights=[m["struct_coefficient"] / count for m in metrics],
                               sample_coefficients=[m["struct_coefficient"] for m in metrics])
    if 'write' in weights:
        terms['write'] = dict(active=weights['write']>0, base_weight=weights['write'],
                             raw_mean=sum(m.get('write',0.) for m in metrics)/count,
                             contribution=write, sample_coefficients=[m.get('write_coefficient',0.) for m in metrics])
    return dict(loss_fm=fm, loss_struct_nominal=struct_nominal, loss_struct=struct,
                loss_temp=temp, loss_write=write, loss_total=fm + struct + temp + write, loss_terms=terms)


def format_losses(values):
    terms = values["loss_terms"]
    parts = []
    for name in ("fm", "struct", "write", "temp"):
        ...
        if not term["active"]:
            ...
            coeffs = term['sample_coefficients']
            parts.append(f"WRITE(raw={term['raw_mean']:.6g},ceiling={term['base_weight']:.3g},"
                         f"sample_w={min(coeffs):.3g}..{max(coeffs):.3g},add={term['contribution']:.6g})")
        elif name == "struct" and term.get("scheme") == "v4_limits":
            coeffs = term["sample_coefficients"]
            parts.append(f"STRUCT(raw={term['raw_mean']:.6g},ceiling={term['base_weight']:.3g},"
                         f"warmup={term['warmup']:.3g},sample_w={min(coeffs):.3g}..{max(coeffs):.3g},"
            ...
            parts.append(f"{name.upper()}(w={term['effective_weight']:.6g},"
                         f"add={term['contribution']:.6g})")
    if values.get("aux_max_share") is not None:
        parts.append(f"STRUCT.grad_share={values['aux_grad_share']:.2%}/{values['aux_max_share']:.0%}")
    elif "core_struct_to_generation" in values:
        ratio, cosine = values["core_struct_to_generation"], values["core_generation_struct_cosine"]
        parts.append(f"core.STRUCT/FM={ratio:.3g}" if ratio is not None else "core.STRUCT/FM=undefined(FM=0)")
        parts.append(f"core.cos={cosine:.3g}" if cosine is not None else "core.cos=undefined")
    if values.get("auxiliary_alignment"):
        parts.append("AUX.conflicts=" + ",".join(name for name, row in values["auxiliary_alignment"].items()
                                                     if row["conflict"]) if
                     any(row["conflict"] for row in values["auxiliary_alignment"].values()) else "AUX.conflicts=none")
    total_label = "TOTAL(nominal)" if values.get("auxiliary_gradient_policy") == "project_conflicts" else "TOTAL"
    return " | ".join(parts) + f" | {total_label}={values['loss_total']:.6g}"

...
from physgen_v42.optim import (EMA, GradientMixer, TRAINING_RECIPE, GATED_RECIPE, REPAIR_RECIPE, load_training, optimizer_for, ramp, restore_rng,
...
def train(cfg, args, device):
    corrected = cfg["train"]["recipe"] in (GATED_RECIPE, REPAIR_RECIPE)
    paired_repair = cfg['train']['recipe'] == REPAIR_RECIPE
    ...
    with job_lock(run_dir / ".training.lock"), TrainingProgress(cfg["train"]["steps"]) as progress:
        ...
        for step in range(start + 1, 1201):
            ...
            mixer = GradientMixer(adapter.parameters() if paired_repair else
            ...
            values = dict(event="update", step=step, phase="fm_struct_write" if paired_repair else "fm_struct" if corrected else ("repair" if step >= 801 else "joint"),

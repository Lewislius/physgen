...
checkpoint="${CHECKPOINT:-${V42_ROOT}/checkpoints/stability_20260917T113347Z_5d0494/step0100}"
suite="${SUITE:-demo}"
variant="${VARIANT:-full}"
case "$variant" in full|base) ;; *) echo 'VARIANT must be full or base' >&2; exit 2 ;; esac
run_name="$(basename -- "$(dirname -- "$checkpoint")")"
step_name="$(basename -- "$checkpoint")"
output="${OUTPUT:-${V42_ROOT}/inference_outputs/${run_name}_${step_name}_ema_${suite}${output_suffix}_seed42_r2_${variant}}"

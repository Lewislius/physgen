...
if __name__ == "__main__":
    print(f"[inference] Python started: {sys.executable}; loading NumPy/PIL ...", flush=True)
import numpy as np
from PIL import Image
if __name__ == "__main__":
    print(f"[inference] NumPy/PIL ready ({time.perf_counter() - _import_started:.1f}s); importing PyTorch ...", flush=True)
import torch
if __name__ == "__main__":
    print(f"[inference] PyTorch ready ({time.perf_counter() - _import_started:.1f}s); loading inference modules ...", flush=True)
...
def build_plan(cfg, args):
    ...
    return dict(version=VERSION, schema_version=3, suite=args.suite, checkpoint=str(checkpoint),
                    ...
                    recipe=dict(RECIPE, variant=variant))
...
def sample_case(model, decoder, case, plan, root, device):
    ...
    folder = root / case["key"]
    full = plan["recipe"].get("variant", "full") == "full"
    ...
    anchor = torch.load(folder / "anchor.pt", map_location=device, weights_only=True) if case["mode"] == "i2v" and full else None
    ...
    with autocast(device):
        ...
        p0 = model.adapter.initialize(text, geo, case["mode"], anchor) if full else None
    ...
    for index, timestep in enumerate(sampler.timesteps):
        ...
        with autocast(device):
            if full:
                ...
                base = model.negative(x, first, text, sigma)
                uncond = model.negative(x, first, negative, sigma)
                result = dict(guided=uncond + recipe["guidance"] * (base - uncond),
                              p15=None, metrics=dict(adapter_disabled=True))
        ...
        relations = state_relations(result, target, geo, model.adapter.s_z, instances) if target is not None and full else None
    ...
    save_tensor(dict(p15=last_p.cpu() if last_p is not None else None, sigma=last_sigma,
                     geometry=geo.geo, adapter_enabled=full), folder / "final_state.pt")
    # GT single-step diagnostics use a separate task/RNG AFTER deployment-only full generation.
    if target is not None and full:
    ...
    save_json(dict(status="completed", signature=signature, case=case, steps=recipe["steps"], sigma_of_final_p15=last_sigma,
                   ...
                   cfg_scheme="native_cfg_plus_bounded_positive_delta" if full else "native_cfg_base_only", p_policy="reset" if full else "disabled",
                   generation_logical_predictions=(3 if full else 2) * recipe["steps"],
                   generation_block_executions=(85 if full else 60) * recipe["steps"],
                   gt_noised_logical_predictions=15 if target is not None and full else 0,
...
def sample(root, device=None):
    ...
    # This phase is a new process; prepare()'s sys.path is not inherited.
    # Use the user's saved code path, just as training and preparation do.
    ...
    try:
        if summary["completed"] != summary["total"]:
            ...
            log(event="wan_source", file=sys.modules["wan.modules.model"].__file__,
                checkpoint=plan["config"]["paths"]["wan_checkpoint"])
...
def main():
    ...
    parser.add_argument("--variant", choices=("full", "base"), default="full",
                        help="Same conditions and seed; base bypasses every adapter operation")

...
from physgen_v42.monitoring import WRITE_VARIANTS
from inference.intervention_report import compare
...
out=ROOT/'analysis/delta_h_quality_20260917/inference_contract_r3'
latents=[]
with torch.no_grad():
    for variant in ('full','base','half','writer5_only','writer15_only'):
        folder=out/variant
        ...
        plan=dict(config=cfg,checkpoint='zero_adapter_contract_only',checkpoint_step=0,ema_updates=0,
                  ...
                  cases=[case],encoders=original['encoders'],
                  recipe=dict(RECIPE,steps=4,variant=variant,writer_scales=list(WRITE_VARIANTS[variant])))
        (folder/'cases.json').write_text(json.dumps(plan,indent=2)+'\n')
        handles=[]
        if variant=='base':
...
for value in latents[1:]: torch.testing.assert_close(latents[0],value,rtol=0,atol=0)
compare([out/v for v in ('full','base','half','writer5_only','writer15_only')],out/'paired_review')
result=dict(purpose='64x64,5 frames,4 solver steps; interface/zero-init check only',
            base_skips_adapter=True,variants=5,automatic_delta_h_reports=True,full_base_max_abs=float((latents[0]-latents[1]).abs().max()),

...
def read_config(path):
    ...
    from .optim import TRAINING_RECIPE, GATED_RECIPE, REPAIR_RECIPE, V4_JOINT_RECIPE
    ...
    if initializer not in ('static_v1', 'condition_trajectory_v1', 'condition_trajectory_gt_v1'):
        raise ValueError('Unknown deployment-condition initializer')
    if initializer != 'static_v1' and cfg['train']['recipe'] not in (GATED_RECIPE,REPAIR_RECIPE,V4_JOINT_RECIPE):
        raise ValueError('The trajectory initializer requires the corrected training recipe')
    if initializer == 'condition_trajectory_gt_v1' and cfg['train']['recipe'] != V4_JOINT_RECIPE:
        raise ValueError('GT initializer requires the explicit v4 joint recipe')
    if cfg["train"]["recipe"] in (GATED_RECIPE,REPAIR_RECIPE,V4_JOINT_RECIPE):
        ...
        if cfg['train']['recipe'] in (REPAIR_RECIPE,V4_JOINT_RECIPE):
            expected['state.initializer'] = ('condition_trajectory_gt_v1' if cfg['train']['recipe']==V4_JOINT_RECIPE
                                             else 'condition_trajectory_v1')
        ...
        if cfg['train']['recipe'] == V4_JOINT_RECIPE:
            expected.update({'loss.struct': .1, 'loss.prior': .02,
                             'train.gt_condition_dropout': .25, 'train.text_dropout': .1,
                             'train.auxiliary_gradient_policy': 'none', 'state.prior_video_pool': 8})

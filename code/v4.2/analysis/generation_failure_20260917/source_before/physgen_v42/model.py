...
def bounded_residual(candidate, base, current, weight, budget, first_known=False, velocity_scale=None):
...
class Writer(nn.Module):
    def __init__(self, width=1664, inner=512, hidden=3072, heads=8):
        super().__init__()
        self.inner = inner
    ...
    def forward(self, h, p, base, sigma, ramp, first_known, site, geo):
        b, _, c = h.shape
...
class Corrector(nn.Module):
    ...
    def __init__(self, args, hidden, text_dim, depth, recompute):
        super().__init__()
        self.core = StateCore(**args, hidden=hidden, text_dim=text_dim, depth=depth, recompute=recompute)
        self.writer = Writer(**args, hidden=hidden)
...
class Adapter(nn.Module):
    def __init__(self, cfg, stats, hidden=3072, text_dim=4096, spatial=576):
        ...
        self.correctors = nn.ModuleDict({str(site): Corrector(
            args, hidden, text_dim, state["depth"], state["checkpoint"]) for site in state["sites"]})
    ...
    def initialize(self, text, geo, mode, anchor=None):
        if mode == "i2v":
            if anchor is None:
    ...
    def auxiliary(self, p0, observations, text, sigma, duration, geo):
        p5 = self.correctors["5"].core(p0.detach(), observations[0].detach(), text.detach(), sigma, duration, 0, geo, self.s_z)

...
mkdir -p ${V3_ROOT}/tmp/trace_p0_t2v ${V3_ROOT}/.cache/huggingface ${V3_ROOT}/.cache/torch
...
RUN_ID=${RUN_ID:-trace-v3-p0-t2v-$(date -u +%Y%m%d-%H%M%S)}
...
export TMPDIR=${V3_ROOT}/tmp/trace_p0_t2v
...
[[ -n ${GLOBAL_PROMPT_OVERRIDE:-} ]] && command+=(--global_prompt_override "${GLOBAL_PROMPT_OVERRIDE}")
[[ -n ${STAGE_PROMPT_OVERRIDE:-} ]] && command+=(--stage_prompt_override "${STAGE_PROMPT_OVERRIDE}")
[[ ${PARSE_ONLY:-0} == 1 ]] && command+=(--parse_only)
[[ ${CHECK_ONLY:-0} == 1 ]] && command+=(--check_only)

printf '[RUN] staged T2V variant=%s: global + time-local stage c+%s\n' \

...
mkdir -p /home/liuzhirui/Project/physGen/code/v3/tmp/trace_m3
...
RUN_ID=${RUN_ID:-trace-v3-m3-$(date -u +%Y%m%d-%H%M%S)}
...
export TMPDIR=/home/liuzhirui/Project/physGen/code/v3/tmp/trace_m3
...
command=(python -B ${INFERENCE_SCRIPT}
  ...
  --dynamic_support_mode ${DYNAMIC_SUPPORT_MODE:-planned}
...
printf '[RUN] TRACE-v3 M3/I2V: 20 planimg demos with their 1280x704 first frames\n'
printf '[RUN] samples=%s cap=%s support=%s output=%s/%s\n' "${SAMPLE_IDS:-all}" "${CAP_MODE:-aggregate_strict}" "${DYNAMIC_SUPPORT_MODE:-planned}" "${OUTPUT_ROOT}" "${RUN_ID}"
exec "${command[@]}"
